...
describe("main route smoke checks", () => {
  ...
  it("renders the admin 404 at /resources instead of any resource UI", async () => {
    // The resource surface is a separate application now. This path must not
    // render it, and must not bounce the user to another origin either — it is
    // simply a page that does not exist in the admin portal.
    renderAt("/resources");

    expect(await screen.findByRole("heading", { level: 1, name: "Page not found" })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Teaching resources" })).not.toBeInTheDocument();
    expect(screen.queryByText("Resource portal")).not.toBeInTheDocument();

    for (const link of screen.queryAllByRole("link")) {
      expect(link.getAttribute("href") || "").not.toMatch(/resources\.tenacitytutoring/);
    }
  });
