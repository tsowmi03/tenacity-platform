...
- Deploy entrypoint: `functions/lib/index.js`.
...
`npm --prefix functions run smoke` currently exports these portal admin callables:
...
- Use indexes already source-controlled in `firestore.indexes.json` where needed.
...
Result: all admin callables listed in this brief were exported locally from `functions/lib/index.js`.
