...
exports.handleUserActions = void 0;
...
exports.handleUserActions = (0, pubsub_1.onMessagePublished)({ topic: 'user-actions',
    region: 'us-central1'
}, async (event) => {
...
async function handleStudentEnrolled(data) {
...
async function handleStudentUnenrolled(data) {
...
async function handleStudentSwapped(data) {
...
async function sendPermanentEnrollmentNotification(classId, studentId, userId) {
    ...
    try {
        ...
        console.log(`Permanent enrollment notification sent to admins: success=${response.successCount}, failure=${response.failureCount}`);
...
async function sendOneOffBookingNotification(classId, studentId, userId, attendanceDocId) {
    var _a;
    ...
    try {
        ...
        if (attendanceDoc.exists) {
            ...
            const sessionDate = (_a = attendanceData.date) === null || _a === void 0 ? void 0 : _a.toDate();
            if (sessionDate) {
        ...
        const response = await messaging.sendEachForMulticast(message);
        console.log(`One-off booking notification sent to admins: success=${response.successCount}, failure=${response.failureCount}`);
        if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
                if (!resp.success) {
                    console.error("Failed token:", tokens[idx], resp.error);
...
async function sendPermanentUnenrollmentNotification(classId, studentId, userId) {
    ...
    try {
        ...
        const response = await messaging.sendEachForMulticast(message);
        console.log(`Permanent unenrollment notification sent to admins: success=${response.successCount}, failure=${response.failureCount}`);
        if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
                if (!resp.success) {
                    console.error("Failed token:", tokens[idx], resp.error);
...
async function sendOneOffUnenrollmentNotification(classId, studentId, userId) {
    ...
    try {
        ...
        const response = await messaging.sendEachForMulticast(message);
        console.log(`One-off cancellation notification sent to admins: success=${response.successCount}, failure=${response.failureCount}`);
        if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
                if (!resp.success) {
                    console.error("Failed token:", tokens[idx], resp.error);
...
async function sendClassSwapNotification(oldClassId, newClassId, studentId, userId) {
    ...
    try {
        ...
        const response = await messaging.sendEachForMulticast(message);
        console.log(`Class swap notification sent to admins: success=${response.successCount}, failure=${response.failureCount}`);
        if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
                if (!resp.success) {
                    console.error("Failed token:", tokens[idx], resp.error);
...
async function handleStudentWeeklyRescheduled(data) {
...
async function sendWeeklyRescheduleNotification(oldClassId, oldAttendanceDocId, newClassId, newAttendanceDocId, studentId, userId) {
    var _a, _b;
    ...
    try {
        ...
        if (oldAttendanceDoc.exists) {
            ...
            const oldSessionDate = (_a = oldAttendanceData.date) === null || _a === void 0 ? void 0 : _a.toDate();
            if (oldSessionDate) {
        ...
        if (newAttendanceDoc.exists) {
            ...
            const newSessionDate = (_b = newAttendanceData.date) === null || _b === void 0 ? void 0 : _b.toDate();
            if (newSessionDate) {
        ...
        const response = await messaging.sendEachForMulticast(message);
        console.log(`Weekly reschedule notification sent to admins: success=${response.successCount}, failure=${response.failureCount}`);
        if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
                if (!resp.success) {
                    console.error("Failed token:", tokens[idx], resp.error);
