...
ClassRollViewData _build({
  ...
  return buildClassRollViewData(
    ...
    attendingStudentIds: attending,
    rollAlreadyMarked: rollAlreadyMarked,
...
void main() {
  group('attendance derivation', () {
    test('a student in the list is here', () {
      final data = _build(attending: const ['s1']);
      expect(data.students.first.attendance, RollAttendance.here);
    });

    test('before the roll is marked, absence means unmarked', () {
      // The stored list cannot tell "not yet marked" from "marked away", so a
      // fresh session must not claim everyone was absent.
      final data = _build(attending: const ['s1'], rollAlreadyMarked: false);
    ...
    test('once the roll is marked, absence means away', () {
      final data = _build(attending: const ['s1'], rollAlreadyMarked: true);
      final ben = data.students.firstWhere((s) => s.studentId == 's2');
  ...
  group('completion', () {
    test('an away student is complete without feedback', () {
      final data = _build(attending: const [], rollAlreadyMarked: true);
    ...
    test('an unmarked student never counts as awaiting feedback', () {
      ...
      final data = _build(attending: const [], rollAlreadyMarked: false);
    ...
    test('an empty roster is not complete', () {
      final data = _build(roster: const [], attending: const []);
