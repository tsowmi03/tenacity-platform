...
The redesign covers three role experiences:
...
The tutor dashboard is the current implementation focus. Its visual first pass,
data adapter, responsive widget tests, bundled fonts, and logo are present. It
remains in progress until the feedback-due and authoritative roll-status
contracts are implemented and a final visual acceptance pass is completed.
...
| F02 | Fonts and licensing | `[x]` | Bricolage Grotesque, Plus Jakarta Sans, and Newsreader are bundled; runtime font fetching is disabled; OFL licence is registered. |
...
| F04 | Shared V3 components | `[ ]` | Build reusable headers, content sheets, section labels, metric tiles, ledger rows, status pills, search bars, filters, empty states, error states, and bottom navigation. |
...
| F06 | Role navigation shells | `[-]` | Tutor bottom navigation has the first V3 treatment. Parent and admin remain legacy. Remove invalid profile tab-index mappings and keep profile navigation as a pushed route. |
...
| --- | --- | --- | --- | --- |
| T01 | Tutor | Dashboard | `[-]` | `TutorDashboardView` and `buildTutorDashboardViewData` implemented; data and final visual gaps remain. |
...
1. Review, test, and commit the current T01 checkpoint on `redesign-v3`.
2. Execute the no-op repository/backend migration through Phase 5 of
   `TENACITY_PLATFORM_MONOREPO_MIGRATION_PLAN.md`.
3. Continue F04–F06 and the T01 visual acceptance work that does not depend on
   the new backend contract.
4. Implement the tutor-session contract, close the T01 data gaps, and deliver
   T02 and T03 as the tutor's core class-to-roll workflow.
5. Deliver T04–T06 using the shared list, search, row, and navigation patterns.
6. Deliver the parent experience P01–P04.
7. Deliver the admin experience A01–A06.
8. Complete shared/detail flows S01–S10 and the release hardening phase.
...
| 20 Jul 2026 | Added the platform monorepo migration plan. | `TENACITY_PLATFORM_MONOREPO_MIGRATION_PLAN.md` defines backend ownership, preserved-history import, protected cutover, shared contracts, rollback, and the tutor-session contract sequence. No repository migration or production deploy has started. |
...
### 5.2 Suggested Flutter organisation
...
| Parent amount due | Parent/admin dashboards and invoices | Define currency/rounding, overdue calculation, multiple invoices, credits, and live refresh after payment. |
...
Cross-repository backend ownership, contract generation, deployment sequencing,
and rollback are defined in `TENACITY_PLATFORM_MONOREPO_MIGRATION_PLAN.md`. The
structural monorepo cutover must complete before the new tutor-session product
...
- Backend/schema work is included only when a verified data-contract gate shows
  it is required for an approved V3 experience.
