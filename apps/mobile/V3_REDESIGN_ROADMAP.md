...
| --- | ---: | ---: | ---: | ---: |
| Reference screens | 14 / 16 | 2 | 0 | 0 |
| Design foundation workstreams | 5 / 8 | 2 | 1 | 0 |
| Supporting/detail workstreams | 9 / 10 | 1 | 0 | 0 |
...
accepted. Its admin console, creation, review, and finalisation half is now
implemented and awaiting product-owner visual acceptance. The two P00 backend
...
| F04 | Shared V3 components | `[-]` | `lib/src/ui/components/` holds `AppHeader`, `DetailHeader`, `MetricTile`, `ContentSheet`, `SectionLabel`, `LedgerRow`/`LedgerRowEmpty`, `AttentionList`, `StatusPill`/`PillButton`, `QuickActionTile`/`QuickActionGrid`, `SearchField`, `ConversationRow`, `SegmentedFilter`, `TimetableRow`, `WeekStrip`, `EmptyStateView`/`ErrorStateView`/`SkeletonBlock`, `AppBottomNavigation`, `AppBottomSheet`/`SheetActions`, `AppConfirmationSheet`, and `OfflineBanner`/`OfflineToast`. `QuickActionGrid` gained a `columns` parameter on 28 Jul 2026: it was hard-coded to two per row, which drew the admin reference's three-up action grid as two tiles plus a half-width orphan. Covered by `test/components_test.dart`. `AppBottomSheet` measures itself from the constraints it is handed rather than from `MediaQuery`, so it behaves inside a modal route and does not collapse where the media query has been replaced rather than extended. `AppConfirmationSheet`, added 29 Jul 2026, gives standard, caution, and destructive decisions one accessible V3 surface and now backs announcements, inbox deletion, roll discard, settings/account deletion, admin people management, class actions, waitlist promotion, and invoice actions. The chat attachment chooser now uses the same bottom-sheet and quick-action primitives instead of its remaining Cupertino action sheet. Remaining: a shared pull-to-refresh wrapper; screens still use `RefreshIndicator` directly. |
| F05 | Role dashboard routing | `[x]` | `DashboardRouter` selects by role, and all three roles now render their own V3 dashboard: `ParentDashboard`, `TutorDashboard` and — from 28 Jul 2026 — `AdminDashboard`. The legacy `HomeDashboard` was deleted on 29 Jul 2026 — see that day's progress-log entry. It had been described as the fallback for an unrecognised role, but `HomeScreen` resolves the role against `destinationsForRole` and shows `_InvalidRoleScreen` before the router is ever built, so the fallback could not fire. The router's `_` branch is now `SizedBox.shrink()`. |
| F06 | Role navigation shells | `[x]` | `home_navigation.dart` defines typed `AppDestination`s and per-role `destinationsForRole`; `home_screen.dart` holds selection as a destination, not an index; profile is a pushed route. The `role == 'tutor'` styling conditionals are gone — `AppBottomNavigation` styles every role. **Two latent defects removed** — both were unreachable in production, and were correct only by coincidence rather than by construction: (1) `profile` mapped to index 5 for parent and tutor against 5-element screen lists, which would have thrown, but nothing ever passed `DashboardDestination.profile`; (2) `notification_service` used `selectTab(4)` for invoice reminders, which is Invoices for a parent but Messages for a tutor or admin — safe only because `invoices.js` sends that type solely to parent tokens. Either would have become a real bug the moment a tab was added or a notification was retargeted. Covered by `test/home_navigation_test.dart`. |
...
| T01 | Tutor | Dashboard | `[x]` | `TutorDashboardView` and `buildTutorDashboardViewData` implemented on the shared component library. The `rolls to mark` count and attention rows now read `Attendance.isRollComplete` rather than inferring from `updatedBy == 'system'`, so an admin editing a session no longer clears a tutor's outstanding roll. Visually accepted by the product owner on 28 Jul 2026. The feedback-due attention row landed 29 Jul 2026, closing the last gap: one `sessionId` query covers the whole week, since every class shares that week's attendance document id, and only students marked `here` without a note are counted. Unmarked rolls take the attention slots first, so a session never appears as both. The read is best-effort and distinguishes "nothing written yet" from "read failed" — the latter raises no row rather than claiming feedback is owed. |
...
| A01 | Admin | Dashboard | `[-]` | `ui/dashboard/admin/admin_dashboard_{data,view}.dart` behind `AdminDashboard`, routed by `DashboardRouter` — admin no longer falls through to the retired `HomeDashboard`. Header carries classes-today, need-action and outstanding metrics; the sheet carries NEEDS ACTION, HAPPENING NOW (falling back to the rest of the day when nothing is running) and the three quick actions. **Roll status is honest about what it knows:** `ROLL 5/6` only once the roll is stamped complete, `NO ROLL` before that — the stored attendance list holds present students only, so an unmarked roll and an all-absent one are indistinguishable and any earlier fraction would be a guess. The roster denominator is the union of the standing roster and whoever was marked present, so a one-off visitor cannot produce `ROLL 7/6`. Per §7: no cover row, no `Approve` on the one-off row, and no room on session rows. Covered by 16 data tests and 9 widget tests at 320/402/430 and text scale 1.3. Inspected on iPhone 16 Pro at 402 × 874 via a temporary preview entrypoint — the simulator is signed in as a parent and must not be signed out, so the screen was rendered against stub models pushed through the real adapter. **Fixed there:** the assigned tutor was appended to the row title, but a real class name plus the roll pill already fills the title, so the tutor fell past the ellipsis and was invisible on every row; it now sits on the subtitle, which room would have occupied had room not been excluded. Regression-tested. **Known cosmetic limit:** long class types still truncate in the title (`Year 11 Advanced Mat…`) — the same `LedgerRow` behaviour already accepted on the parent and tutor dashboards, so it is left alone rather than restyled a shared component unilaterally; raise at acceptance if it should change. Visually accepted by the product owner on 29 Jul 2026. New enrol opens its own picker from 29 Jul 2026: student, then class, then enrolment type, ending in the same shared write as the class-side flow. The class list is annotated against the chosen student, so a class they are already on is shown greyed rather than failing at the write, and seats are reported before the choice rather than after. **Remaining:** a per-class route for session and roll rows. |
| A02 | Admin | Classes | `[x]` | `ui/timetable/admin/admin_classes_{data,view}.dart`, rendered by `TimetableScreen` for admins — the old timetable body was deleted on 29 Jul 2026. A one-day time-grouped ledger under the shared `WeekNavigator` and `WeekStrip` (MOB-7, 17 Aug 2026 — it paged by day and named no week until then), with `RUNNING` / `NO ROLL` / `DONE` / `FULL` / `N SEATS` / `CANCELLED`, seats counted against the roster **plus** this week's visitors, and the slot containing the current moment marked `Now`. A row expands in place to list its students. Every row now opens V3 class options, roster, existing-student picker, enrolment type, tutor assignment, waitlist/promotion, feedback, cancellation, and add-class sheets. Mark roll opens T03. Checked Firestore transactions reject stale tutor or weekly-booking editors and write only the intended fields, so neither can replace a concurrent capacity, roster, cancellation, or roll change. There is no capacity editor; the removed whole-model tutor update was the path that could overwrite capacity. Standing tutor propagation uses stored session dates rather than week-number suffixes across terms. Class creation now calls the authoritative `adminCreateClass`: the class and all remaining active-term attendance sessions are written in one backend transaction. The client supplies a stable class id, and a retry after a lost response reconciles only when the stored creation values match, so it cannot overwrite concurrent class or session state. The reference's `Rooms` half of the toggle is excluded (one room), replaced by a `Tutors` grouping that lists a co-taught class under each tutor and sorts an `Unassigned` bucket last. S08 has focused data, sheet, checked-write, idempotency, and lifecycle coverage in the passing mobile and backend suites. Visually accepted by the product owner on 29 Jul 2026. |
...
| A04 | Admin | Users | `[x]` | `ui/users/admin/admin_users_{data,view}.dart` behind `UsersScreen` for admins — the legacy list was deleted on 29 Jul 2026. Parents/Students/Tutors tabs over a searchable directory: parents carry their children and token balance, students their year and subjects, tutors their role. Search matches name **and** subtitle, so a parent is findable by their child's name. Counts describe the whole directory rather than the filtered tab, so searching does not look like people have disappeared. **Status is overdue-only**, derived from the family's own unpaid invoices past their due date — the reference's `ACTIVE` and `TRIAL` have no source anywhere in the data and are not invented, and the `+` account-creation button is out of scope (§7/§11). The invoice read is best-effort: if it fails nobody is marked, rather than families being wrongly accused. Opening a person routes into the V3 `AdminPersonScreen` (S05), so lesson tokens, enrolments, invoice PDF, unenrolment and account removal keep their behaviour and confirmations. Students carry no account, so nothing admin-only can be opened from their row. Covered by 16 data tests and 11 widget tests at 320/402/430 and text scale 1.3. Inspected on iPhone 16 Pro across the Parents and Students tabs. Visually accepted by the product owner on 29 Jul 2026. |
| A05 | Admin | Messages | `[x]` | Covered by the role-agnostic P03 inbox, S04 thread, contact picker, and V3 attachment sheet. Search, unread state, failure-safe deletion, attachments, pending uploads, and receipts are shared rather than reimplemented for admins. Confirmed against the admin reference by the product owner on 29 Jul 2026; no code change was needed. |
| A06 | Admin | Invoices | `[-]` | `ui/invoices/admin/admin_billing_{data,view}.dart` behind `AdminBillingScreen`, now the admin `invoices` destination. Outstanding headline, unpaid/overdue counts, All/Overdue/Unpaid/Paid filters, an overdue ledger graded red past a week and amber before it, and recent payments. `AdminInvoiceView` is now a V3 operational console reached through `View all invoices` and any invoice row. It preserves filter, sort, search, multi-select, bulk mark-paid/delete, invoice detail, PDF retrieval, and the create/review/finalisation chain. The create wizard uses a V3 due-date sheet; review keeps editable/add/remove line items, discounts, override totals with matching Xero adjustments, notes, and duplicate-submit protection. Each creation attempt has a stable request id; a backend transaction ledger fingerprints the payload and returns the same invoice and invoice number after an ambiguous retry, while rejecting a changed payload under the same id. A successful first creation alone sends the notification and writes the mobile audit event. Failed paid-status writes now propagate, and partial bulk failures exit selection and reload the console before reporting which rows failed. **Four reference elements are excluded** (§7/§11): `Send N reminders`, the per-row `Follow up`/`Remind` actions and the `reminded ×N` count — reminders are automatic and record nothing, so none could report what they did — and `Visa ····4242`, whose contract is the deferred P00 gate. In their place each overdue row shows **when the next automatic reminder falls**, derived from `invoiceReminderScheduler`'s own rule (due−7d, due date, then every 7 days) and pinned by tests that fail first if that schedule changes. A06 and S09 have focused data, widget, idempotency, and lifecycle coverage in the passing mobile and backend suites. Visually accepted by the product owner on 29 Jul 2026. **Remaining:** a live PDF smoke test. |
...
| S04 | Chat creation and thread | `[x]` | The thread is on the V3 palette: navy header carrying the same squircle identity as the inbox row that opens it, blue/blue-50 bubbles, tokenised date separators, read receipts, typing indicator and composer. Text, image and file sending, drafts, pending states, upload progress and offline guards are untouched. The attachment chooser now uses `AppBottomSheet` and `QuickActionGrid`, replacing the last Cupertino action sheet. The contact picker is rebuilt as `ui/messaging/new_chat_{data,view}.dart` behind `new_chat_screen.dart`: a detail header with a live contact count, the inbox's search field, and role-grouped identity rows over a content sheet, with skeleton, retry, no-match and offline-empty states. The parent-to-parent restriction moved into the pure adapter, where it is now tested. Covered by data and widget tests at 320/402/430 and text scale 1.3, plus a dedicated attachment-sheet test. **No reference design exists for either screen** — the design files only include the inbox — so both extend the established language; revisit if a thread or picker design is produced. |
| S05 | User details and management | `[x]` | Tutor-facing halves done: `ui/users/tutor/student_detail_{data,view,screen}.dart` gives a student's record — a **DETAILS** block (year, subjects, latest progress status), latest feedback in full with the history one tap away, their classes with the viewing tutor's own marked `YOURS`, and their family with the primary contact marked and sorted first, routing into the parent. `parent_detail_{view,screen}.dart` gives a parent's record — contact details with copy, children routing into the student, and a message shortcut. **Deliberately narrower than the admin screen:** no lesson tokens, no invoice and no account removal. Tutors cannot read invoices at all (the rules allow admins and the parent only), so the retired screen's billing section could only ever render empty for a tutor while still issuing the denied read. **Navigation is bounded:** `person_routes.dart`'s `pushPersonRoute` keeps one open instance per person, so the student ↔ parent link cannot be used to stack the same two records repeatedly — linking to someone already open returns to them instead. Covered by 27 data tests plus a dedicated navigation-depth suite. The admin half landed 28 Jul 2026: `ui/users/admin/admin_person_{data,view,screen}.dart` replaces `UserDetailScreen` behind A04 — contact details with a message shortcut, the lesson-token balance with an edit action, children expanding to their enrolments with per-student unenrol, this family's invoices with PDF entry, and a danger zone. The lesson-token editor is now a validated V3 sheet, and destructive actions use the shared confirmation sheet. Every mutation keeps its existing `OfflineActionGuard`; the removal warning is written per role, so removing a parent says in as many words that it takes their children too. Invoices are filtered to this person, since the controller holds whatever was last loaded. Covered by 13 data tests plus 4 lesson-token sheet tests. Verified on iPhone 16 Pro. **Fixed there:** the invoice rows used `LedgerRow`, which reserves a leading time column an invoice has nothing to fill, leaving an empty gutter. Visually accepted by the product owner on 29 Jul 2026. `UserDetailScreen` was deleted on 29 Jul 2026. |
| S06 | Student feedback history | `[x]` | `ui/feedback/feedback_history_{data,view}.dart` behind `feedback_screen.dart`, shared by every role and reached from the parent dashboard, the tutor directory, a student's record and a push notification. Notes are newest-first with `<tutor> · <class>` attribution — now in brand blue rather than uniform grey, via the shared `FeedbackAttribution` widget also used on the student record — plus relative dates, an unread emphasis, and the progress status now that the tutor-session contract records one. Mark-as-read is skipped offline rather than queued, and is not re-issued on rebuild. Admin creation moved onto the V3 sheet with the same validation, trimming and busy-state guarantees. Each composer now creates one feedback UUID before opening and reuses it for retries, while `FeedbackService` writes that id directly, so a lost success response cannot create a duplicate note. Covered by the data and screen suites, including an ambiguous-retry regression test. |
...
| S08 | Admin class-management flows | `[x]` | Implemented end to end on V3 sheets: class options, roster and weekly bookings, existing-student search, one-off/permanent enrolment choice, feedback creation, tutor assignment, waitlist management and promotion, cancellation confirmations, add class, and T03 roll routing. One-off is omitted when the displayed week has no generated session. Checked, field-narrow transactions reject stale tutor and roster editors without replacing capacity, cancellation, roll, or other concurrent fields. Permanent propagation uses stored session dates across terms. Add class validates a positive duration and calls authoritative `adminCreateClass`, which atomically creates the class and the active term's remaining sessions behind a stable client class id. Enrollment, deletion, creation, generation, swap, absence, and paid-write style failures now reach the surface instead of returning false success. Ambiguous swaps close and refresh both affected class views; waitlist promotion locks the whole sheet until its write and reload settle. There is no capacity editor; removing the old whole-model tutor update removed the capacity-overwrite path carried from A02. Covered by focused data, sheet, checked-write, idempotency, and lifecycle tests. The source scan finds no raw Material or Cupertino dialog, popup, or modal route outside the shared V3 helpers. Visually accepted by the product owner on 29 Jul 2026. |
| S09 | Invoice payment and creation details | `[-]` | Parent payment surfaces are complete and visually accepted: Stripe sheet orchestration, pay-now/pay-all intent reuse, success/cancel/failure/pending outcomes, paid-state reconciliation, PDF coalescing/open failures, and retryable invoice loading. The admin half is now implemented: V3 console/detail, search/filter/sort/multi-select and bulk actions, PDF entry, family/student/session selection, V3 due-date picker, line-item review/edit/add/remove, override adjustment, notes, duplicate-submit guard, and finalisation. Invoice creation keeps one request id and immutable payload across retries; the backend ledger returns the original invoice after a lost success response and rejects id reuse with changed data. Successful finalisation closes the whole wizard, a rejected mark-paid write stays open and reports failure, and a partial bulk failure reloads the console before reporting the affected rows. Covered by focused mobile and backend idempotency, console, and lifecycle tests. Visually accepted by the product owner on 29 Jul 2026. **Remaining:** a live PDF smoke test. The P00 card-brand/last4 contract is deliberately deferred to the end of the redesign (product decision, 28 Jul 2026) rather than blocking this row — see §7. |
...
**Phase 4, the admin experience, is visually accepted as of 29 Jul 2026.** All
six admin reference screens (A01–A06), plus S05, S08 and S09 underneath them,
were confirmed by the product owner inside the complete `HomeScreen` shell.
**Every legacy role surface has been retired** — parent, tutor and admin each
render V3 for the dashboard, classes, users and billing, and the legacy
screens behind them were deleted on 29 Jul 2026.

**No reachable legacy interface remains in the mobile app.** S08's admin
class-management flows and S09's invoice console, creation, and review chain
now use V3 screens and sheets. Confirmations across announcements, inbox,
class roll, settings, people, classes, and invoices use the shared V3
confirmation sheet. A source scan finds no Material or Cupertino dialog or
action sheet left in `lib/src`, and the only `showModalBottomSheet` call is
inside the shared `AppBottomSheet` helper. The unused pre-redesign
student-search dialog and its package dependency have also been removed.

**The release-blocking rules deployment is done** (29 Jul 2026), and with it the
feedback-due row and the New enrol picker. A01 and A06 stay `[-]` on one
cosmetic item each; nothing outstanding blocks a release.
...
2. ~~Product-owner visual acceptance of A01, A02, A04, A05, A06, S05, S08 and
   S09.~~ **Done 29 Jul 2026.**
3. ~~Deploy the Firestore rules change.~~ **Done 29 Jul 2026**, to
   `tenacity-tutoring-b8eb2`, after the 18-test rules suite passed against the
   emulator. `source-baseline.json` re-captured the same day.
4. ~~Live invoice PDF smoke test.~~ **Done 29 Jul 2026.**
5. ~~Direct New enrol → student → class picker for A01.~~ **Done 29 Jul 2026.**
6. ~~T01's feedback-due attention row.~~ **Done 29 Jul 2026.**
7. Land the two parent backend contracts when picked back up: payment card
   brand/last4, and confirmation of the amount-due rounding rules. Deferred to
   the end of the redesign by product decision and not currently scheduled.
8. Release hardening (§6 Phase 6): update README/release notes, run the full
   gate one more time, and go through a controlled internal build and
   role-based smoke test.
9. Remaining cosmetic gaps, neither release-blocking: A01's per-class route for
   session and roll rows, and F04's shared pull-to-refresh wrapper.
...
| --- | --- | --- |
| 17 Aug 2026 | MOB-7: gave A02 the shared week header and put its rosters on the timetable. | A02 was the only timetable that named no week and the only one without the `WeekNavigator`/`WeekStrip` pair, so an admin could not see which term week they were looking at. The arrows now page weeks and the strip picks the day, which **deleted** `_changeAdminDay`, its `_termWeekForDate` inverse of `startOfTermWeek`, and `TimetableController.setWeek` — all three existed only because a day pager could step out of the loaded week's attendance. `buildAdminClassesViewData` now builds the whole week before filtering to the day, since the strip's dots and the header's count need the days that are not on show. Seeing who was in a class had cost two taps and a document read per student behind the enrolments sheet; a row now expands to its roster, fed by one `fetchAllStudents` collection read — the one the student picker already makes — resolved into names in the same standing-then-visiting, alphabetical order `buildAdminRosterEntries` uses. A roster id whose student document does not resolve is dropped from the names but still counted in the seats, and the expanded row says the names are loading rather than claiming the class is empty. The expand control is a sibling of the row's tap target, so opening the roster and opening class options stay separate. Each student is listed with their year and subject, since a name alone answers little in the `Years 5–10` classes — six year groups doing two subjects in one room, and most of the timetable. The same detail went onto the enrolments sheet and the student picker; the picker had shown a bare `9` for a Year 9 student and no subject, on the one screen where picking the wrong student enrols them into a class meant for another year. `yearLabelFor` moved from `class_roll_data.dart` to `dashboard_formatting.dart` to be reachable from all three, joined by `studentSubjectLabel` and `studentYearAndSubjects`; the subject label is derived from `formatDashboardClassType` with the leading year stripped rather than from a second table, so the two cannot drift. Suite 1026, all passing; 30 new or rewritten tests across admin classes and the shared formatters, including an expanded roster of long names at 320pt and text scale 1.3. Not yet visually accepted on device. |
| 29 Jul 2026 | Deployed the release-blocking rules; closed the New enrol picker and the feedback-due row. | The `validFeedbackCreate()` change went to `tenacity-tutoring-b8eb2` after the 18-test rules suite passed against the emulator, and `source-baseline.json` was re-captured against the deployed hash. `docs/operations/pending-rules-deployment.md` is deleted, its purpose served. A01's New enrol now opens student → class → enrolment type instead of routing to Classes; the class-side and dashboard flows share one write, so the offline guard, already-enrolled handling and post-write reload cannot drift. T01's feedback-due row reads one `sessionId` query for the whole week — every class shares that week's attendance document id — counts only students marked `here` with no note, and yields the attention slots to unmarked rolls so a session never appears twice. A failed read raises no row rather than claiming feedback is owed. 884 mobile tests (15 new), analysis at 0 errors and 0 warnings, clean formatting, successful web build. |
| 29 Jul 2026 | Finished the last reachable legacy mobile surfaces. | S08's admin class options, roster, enrolment, tutor, waitlist, feedback, cancellation, and class-creation paths now use V3 sheets; S09's admin invoice console, detail, due-date, creation, and review chain now uses V3 surfaces. Shared confirmations replaced direct dialogs across announcements, inbox, roll, settings, people, classes, and invoices, and the chat attachment chooser replaced the last Cupertino action sheet. Safety work added atomic class creation, invoice and feedback retry identities, checked-write error propagation, whole-route mutation locks, and refresh-after-ambiguous or partial results. The old student-search helper and `multi_select_flutter` dependency are gone. A source scan finds no Material or Cupertino dialog or action sheet left in `lib/src`, and the only `showModalBottomSheet` call is inside the shared `AppBottomSheet` helper. Full validation: 869 mobile tests, 589 backend unit tests, 84 backend emulator tests, analysis at 0 errors and 0 warnings (3 existing info findings), clean formatting, and a successful production web build. **Follow-up:** product-owner visual acceptance of the completed admin surfaces and a live invoice PDF smoke test; these are acceptance checks, not remaining legacy code. |
| 29 Jul 2026 | Deleted the dead legacy code the redesign had left behind. 15 files, 2,848 net lines. | **Found by audit, not planned.** Three groups. (1) **The legacy role bodies**, which the roadmap and three code comments described as "reachable only by an unrecognised role" — they were reachable by nobody. `HomeScreen` resolves the role through `destinationsForRole` and returns `_InvalidRoleScreen` when it comes back empty, which is exactly the unrecognised-role case, and it does so before the shell builds. So `HomeDashboard`, the legacy `UsersScreen` list, `UserDetailScreen` (its only call site was inside that list) and the legacy `TimetableScreen` body could not render for any role. Phase 6's "remove only after role parity is proven" gate was therefore already satisfied — there was no role left to prove parity for. (2) **Payslips**, deleted by product decision: both screens plus `PayslipController`, `PayslipService` and `Payslip`, and the provider in `main.dart` that constructed the controller on every launch. Entirely unreferenced, with no Cloud Function behind it. (3) **Stray unreferenced files**: `debug_log_screen.dart` (no entry point, despite being carried in §11 as a dev utility), `user_service.dart`, `student_feedback_expandsion_card.dart`, `student_names.dart`, three helper stubs (`action_option`, `invoice_list_tile`, `notification_navigation`), and a **1-byte empty `lib/controllers/invoice_controller.dart`** in a stray top-level directory that shadowed nothing. `TimetableScreen` went 3,940 → 3,199 lines. **No behaviour change and no test change:** the suite is 762 before and after, because nothing deleted had a single test — which is itself the evidence it was unreachable. Full gate passes (format, analyze at 0 errors/warnings, 762 tests, `flutter build web`). **Follow-up completed later on 29 Jul 2026:** the S08 class-management sheets and S09 invoice console chain are recorded in the row above. |
| 29 Jul 2026 | Split the roll from the booking list, so two tutors can mark one session. | Most classes are taught by two tutors and neither could mark from their own phone without losing the other's work. One root cause, three symptoms: the roll wrote `attendance` — the array everything else reads as *who is booked* — with only the students who turned up. So marking a student **Away un-booked them**, freeing a seat that was not free and firing a spurious `student removed from class` alert at admins, and `submitSession` wrote the whole document, so the second tutor's save replaced the first's. `Attendance.marks` now records the roll and the roll no longer touches `attendance`. Each mark is written as its own `marks.<id>` field path, so co-tutors merge. Completion is derived from marks covering the roster rather than stamped, which also removes the old limitation behind `ROLL n/m`: an empty list could not distinguish *nobody marked* from *everybody away*, so the fraction had to be withheld until the roll was stamped — it is now honest mid-roll. Feedback moved to a deterministic id (`<sessionId>_<studentId>`) and re-reads what exists immediately before writing, so two tutors writing about the same student cannot send the family two notes. **Backend:** the attendance report was reading the same array as attendance and would have reported every session fully attended; it now prefers `marks` and falls back to the array for pre-marks history. `oneOffBookings` stays on the booking list. **Not in the plan, found by audit.** Suite 745 → 762 mobile, 576 → 586 backend. Verified on device against production: marked one student from one entry point, saved, reopened the roll fresh and marked a second — both marks survived, and the class still read `4/4 seats` after the Away mark. **Follow-up:** the backfill (`npm run dryrun:roll-marks`) has not run — local ADC needs `gcloud auth application-default login`. |
| 29 Jul 2026 | The roll no longer pre-marks everyone present. | A consequence of the above worth calling out, since T03 is already visually accepted. The roll screen used to render every booked student as **Here** before anyone had touched it, because "booked" and "present" were the same field. They are now distinct, so a fresh roll opens with all students **unmarked** and the tutor marks each one. This is what makes `N of M complete` and `ROLL n/m` true, but it costs a tap per student in the common case where everyone attends. No `Mark all here` affordance was added — flagged for a product decision rather than assumed. |
...
- [x] Preserve add/edit class, tutor assignment scope, cancellation, attendance,
  roster editing, waitlist management, and promotion. All now use V3 sheets.
- [x] Provide safe conflict handling for tutor, capacity, and concurrent edits.
  Tutor and weekly-booking writes compare the expected identifiers and update
  only their own fields. There is no capacity editor, and the old whole-model
  update path that could replace capacity has been removed.
...
- [x] Preserve parent/student/tutor details, token editing, feedback navigation,
  invoice PDF, unenrolment, and account removal. These now use the V3
  `AdminPersonScreen` and shared sheets.
- [x] Keep destructive actions admin-only and require clear confirmation.
  They stay behind the same authority checks and now use the shared V3
  confirmation sheet.
...
- [x] Preserve existing filter, sort, search, multi-select, bulk-action, draft,
  review, line-item editing, finalisation, and PDF behaviour. The full console,
  detail, creation, due-date, review, and finalisation surfaces now use V3.
...
- [x] Reskin login and password-reset states.
...
- [x] Reskin user details and admin people-management sheets/dialogs.
- [x] Align feedback history and add-feedback screens with T03.
- [x] Reskin parent booking, swap, waitlist, and confirmation flows.
- [x] Reskin admin class editor, tutor assignment, cancellation, roster, and
  waitlist flows.
- [x] Reskin invoice payment, creation, review, and PDF-entry surfaces.
- [x] Reskin profile, edit profile, password, settings, and sign-out flows.
- [x] Decide whether payslips remain excluded. They were removed on 29 Jul 2026.
  Restoring them requires separately approved navigation, permissions, V3
  designs, and tests.

Exit criteria: every route reachable from a V3 screen uses the V3 system or has
an explicitly documented temporary exception. **Implementation exit met on
29 Jul 2026.** Product visual acceptance remains tracked under S05, S08, and
S09.
...
- [x] Remove obsolete legacy dashboard code only after role parity is proven.
  **Done 29 Jul 2026.** The gate turned out to be already satisfied: the legacy
  bodies were guarded on an unrecognised role, which `HomeScreen` refuses before
  the shell is built, so there was no role left to prove parity for. 15 files
  deleted, 2,848 net lines. See that day's progress-log entry.
- [ ] Update README/release notes and document any backend migrations.
- [x] **Deploy the Firestore rules change before the build that depends on it.**
  **Done 29 Jul 2026.** `validFeedbackCreate()` used `hasOnly()`, so the
  deployed rules rejected the `classId`/`sessionId`/`progress` keys the tutor
  roll writes and every roll submission would have failed. Released to
  `tenacity-tutoring-b8eb2` after the 18-test rules suite passed against the
  emulator.
- [x] Re-capture `backend/firebase/inventory/source-baseline.json` once those
  rules are deployed. **Done 29 Jul 2026**, against the deployed hash.
...
| Roll completion | Tutor/admin dashboards and classes | **Resolved and implemented 28 Jul 2026; reworked 29 Jul 2026.** The roll and the booking list are now separate fields. `Attendance.marks` (`{studentId: here\|away}`) records the roll; `attendance` keeps its original meaning as who is booked, which is what capacity, one-off eligibility, parent views and the backend reminders read. Completion is **derived** — `isRollCompleteFor(roster)` asks whether every rostered student carries a mark — so a partial save can no longer claim a roll is finished, and adding a student reopens it. `rollCompletedAt`/`rollCompletedBy` remain for attribution and as the fallback for documents written before marks existed. Marks are written one field path at a time (`marks.<studentId>`), so two tutors on one class merge instead of overwriting. No rules change was needed — staff already hold write access to attendance. |
| Feedback due/completion | Tutor dashboard and roll | **Partly implemented 28 Jul 2026.** `StudentFeedback` now carries `classId`, `sessionId` and `progress`, so feedback is session-identified and the roll screen shows `N of M complete` and what is outstanding. The rules this depends on were deployed 29 Jul 2026. **Resolved 29 Jul 2026:** the tutor dashboard's feedback-due attention row now reads one query per week — every class shares the week's attendance document id, so a single `sessionId` lookup covers them all — and counts only students marked `here` who have no note yet. A failed read raises no row rather than accusing a tutor of owing feedback. |
...
- `debug_log_screen.dart` was **deleted** on 29 Jul 2026. It had been carried as
  a developer utility outside the V3 visual scope, but an audit found it had no
  entry point anywhere in the tree, so it was not reachable to be useful.
  Recover from git history if a log viewer is wanted again.
- Payslips were **deleted** on 29 Jul 2026 (product decision), not merely kept
  out of navigation. Both screens, `PayslipController`, `PayslipService` and
  `Payslip` are gone, along with the provider that constructed the controller on
  every launch. The feature was entirely unreferenced and had no Cloud Function
  behind it. Recover from git history if product scope restores it.
