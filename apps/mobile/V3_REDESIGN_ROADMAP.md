...
What keeps A01, A06 and S09 at `[-]` is no longer visual acceptance — it is the
three concrete items below (New enrol picker, PDF smoke test, and the T01
feedback query), plus the release-blocking rules deployment that applies
regardless of screen status.
...
3. **Deploy the Firestore rules change before the build that depends on it.**
   Release-blocking — see item 7 and
   [`docs/operations/pending-rules-deployment.md`](../../docs/operations/pending-rules-deployment.md).
4. Run a live invoice PDF smoke test (A06/S09's one remaining item).
5. Build the direct New enrol → existing-student picker for A01; its
   class-side enrolment route is already complete.
...
8. Release hardening (§6 Phase 6): update README/release notes, re-capture
   `source-baseline.json` after the rules deploy, run the full gate one more
   time, and go through a controlled internal build and role-based smoke test.
