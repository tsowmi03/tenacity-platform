...
**The legacy that remains is all admin-only and all tracked**, in two clusters:
the class-management dialogs inside `timetable_screen.dart` (S08 — the roster
half of the students editor, tutor assignment, waitlist, add-class and the
enrol-student dialog) and the invoice console chain `AdminInvoiceView` →
`AdminCreateInvoiceScreen` → `AdminReviewInvoiceScreen` (S09). Both are entered
from finished V3 screens, so an admin still crosses a visual seam mid-task.
Nothing else in the tree is legacy; a shared destructive-confirmation surface
(F04) is the one remaining pattern gap on otherwise-accepted V3 screens.
