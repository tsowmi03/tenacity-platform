// forked from design-sync lib/source-kit.mjs — the synth-entry generator used
// `export * from` only, which never forwards default exports; this repo's
// components are all `export default <Name>`, so nothing landed on
// window.<GLOBAL> ([BUNDLE_EXPORT] 43/43). Added a default-export re-export
// alongside the `export *` line for each synthesized entry file.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // `export * from` (ES module semantics) never forwards default exports —
    // this repo's components are all `export default <Name>` (or `export
    // default function <Name>`), so the un-forked generator produced an
    // entry with zero of those 43 components actually reachable on
    // window.<GLOBAL>. Re-export each file's default under its declared
    // name alongside the `export *` so both named and default exports land.
    const DEFAULT_RX = /^\s*export\s+default\s+(?:function\s+|class\s+)?([A-Za-z_$][\w$]*)\b/m;
    const entryLines = [];
    for (const p of comps) {
      entryLines.push(`export * from ${JSON.stringify(p)};`);
      const m = DEFAULT_RX.exec(readText(p));
      if (m) entryLines.push(`export { default as ${m[1]} } from ${JSON.stringify(p)};`);
    }
