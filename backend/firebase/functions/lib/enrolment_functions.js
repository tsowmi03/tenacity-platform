...
exports.acceptPendingEnrolment = void 0;
const https_1 = require("firebase-functions/v2/https");
const logger = require("firebase-functions/logger");
const admin = require("firebase-admin");
const params_1 = require("firebase-functions/params");
const email_functions_1 = require("./email_functions");
...
// -----------------------------------------------------------------------------------
exports.acceptPendingEnrolment = (0, https_1.onRequest)({
    ...
    try {
        ...
        const enrolmentId = req.query.enrolmentId;
        if (!enrolmentId) {
        ...
        const selectedClassIds = Array.isArray(enrolmentData.classes)
            ? enrolmentData.classes.map((c) => c.id)
            : [];
        ...
        try {
            ...
            if (error.code === "auth/user-not-found") {
                ...
                await (0, email_functions_1.sendParentWelcomeEmail)(parentDocData.email, parentDocData.firstName);
            }
            else {
                throw error;
            }
        ...
        try {
            ...
            logger.info(`Successfully processed enrolment ${enrolmentId}: created student doc [${newStudentRef.id}] and enrolled in classes`);
            const studentName = `${studentDocData.firstName} ${studentDocData.lastName}`;
            const classArray = enrolmentData.classes || [];
            const classes = classArray.map((c) => `${c.day} @ ${c.startTime}`).join(", ");
            const subjects = studentDocData.subjects.join(", ");
            await (0, email_functions_1.sendParentEnrolmentAcceptedEmail)(parentDocData.email, studentName, classes, subjects);
        }
        catch (error) {
